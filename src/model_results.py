...
class ResultsTab(QWidget):
    def __init__(self):
        ...
        self.model_ui()  # Inicializamos la UI

        self.current_graph = None  # Variable para almacenar el gráfico actual
    ...
    def update_tab(self, description, r2, mse, formula, plot_data, coef, intercept, input_columns, output_column, warning_text=""):
        """Actualiza los valores en la UI con la información proporcionada."""
        ...
        self.description_text.setText(f"'{description}'")
        ...
        # Mostrar una advertencia, si es que se proporciona
        if warning_text:
        ...
        # Actualizar el gráfico de regresión si los datos están disponibles
        if plot_data is not None:
            self.plot_regression_line(plot_data)
        else:
            self.display_no_plot_message()

        # Crear los campos de entrada para las predicciones
    ...
    def plot_regression_line(self, plot_data):
        """Genera o actualiza la gráfica de regresión."""
        if self.current_graph:  # Eliminar el gráfico anterior si existe
        ...
        # Guardamos el gráfico actual para poder eliminarlo si es necesario
        self.current_graph = canvas

    def display_no_plot_message(self):
        """Muestra un mensaje indicando que no es posible generar la gráfica."""
        if self.current_graph:  # Si hay un gráfico previo, lo eliminamos
            self.current_graph.deleteLater()
        
        # Crear un mensaje en lugar del gráfico
        no_plot_label = QLabel("No se puede generar la gráfica debido a la falta de datos suficientes o válidos.")
        no_plot_label.setStyleSheet("color: red; font-weight: bold; padding: 10px;")
        self.layout.addWidget(no_plot_label)

        # Guardamos este mensaje en lugar del gráfico actual
        self.current_graph = no_plot_label
